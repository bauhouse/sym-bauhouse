...
<xsl:for-each select="content/entry[fields/section = 'Imagine' and fields/sort != '']">
...
<div id="sectionhead">
<h2>Imagine<xsl:for-each select="content/entry[@handle = $entry]">
<xsl:text> : </xsl:text>
...
<xsl:for-each select="content/entry[fields/section='Imagine']">
<xsl:sort select="fields/sort"/>
<xsl:choose>
<xsl:when test="@handle = $entry">
				<div class="entry">
					...
					<div class="entry_body">
						...
						<xsl:if test="fields/sort &lt; $section-count">
							<xsl:for-each select="/data/content/entry[fields/section='Imagine' and fields/sort = $sort-next]">
								<p class="entry_info">
									Next Page:
						...
						<xsl:if test="fields/sort = $section-count">
							<xsl:for-each select="/data/content/entry[fields/section='Design' and @handle = 'design']">
								<p class="entry_info">
									Next Section:
									<a href="{$root}/{@handle}/">
									<xsl:value-of select="fields/title"/>
...
<xsl:when test="$entry='' and position() = 1">
				<div class="entry">
					...
					<div class="entry_body">
						<h2><xsl:value-of select="fields/title"/></h2>
						<xsl:for-each select="fields/body/*"><xsl:copy-of select="."/></xsl:for-each>
						<xsl:for-each select="/data/content/entry[fields/section='Imagine' and fields/sort = 1]">
							<p class="entry_info">
								Next Page:
								<a href="{$root}/{$current-page}/{@handle}/">
								<xsl:value-of select="fields/title"/>
