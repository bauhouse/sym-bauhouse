	...
	<div class="content_thumbs">
		<xsl:for-each select="portfolio/entry[preview/filename != '']">
			<xsl:if test="preview != '' and position() &lt;=7">
				<a href="{$root}/design/{media/item/@handle}/{title/@handle}/" title="{title}"><img class="preview_off" src="{$workspace}{preview-off/@path}/{preview-off/filename}" width="107" height="107" alt="{title}" /><img class="preview_over" src="{$workspace}{preview/@path}/{preview/filename}" alt="{title}" /></a>
			</xsl:if>
		</xsl:for-each>
